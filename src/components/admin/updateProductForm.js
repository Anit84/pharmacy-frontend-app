import React,{useEffect, forwardRef,useState, useImperativeHandle,Fragment} from 'react';
      ...
      const { productName,productDescription,productPrice, productMeasure, productCategory, productPercent} = formData;
       ...
       const handleProductSubmit = async() => {
        ...
        if(productName.length <=0 || productDescription.length <=0 ||productPrice.length <=0||productMeasure.length <=0||productCategory.length <=0 ||  productPercent < 0 || imgSrc.length <=0){
            ...
            try {
                setAuthToken(localStorage.token)
                await axios.patch(`${baseUrl}/api/user/product/update`, formDataHere, config)
                ...
                <Modal.Body>
                    ...
                    <Form onSubmit={e => e.preventDefault()}>
                        <Form.Row className="mb-2" >
                            <img alt="Product" src={imgSrc} style={{height:'20rem', width:'20rem'}} className="img-thumbnail" width="100%" />
                            <Form.Group>
